...
class RecommendArticles():
    ...
    def __getRecommendations__(self):        
        ...
        if max(self.__similarUsersID__['Articles Similarity']) > 3:            
            for idx in self.__similarUsersID__['Similar_Users'].index:
                ...
                if len(self.__keywords__) != 0:
                    v_articles['Keywords Similarity'] = 0
                    for keyword in self.__keywords__:
                        v_articles['Keywords Similarity'] = v_articles['Keywords Similarity'] \
                                                             + v_articles['topic_keywords'].apply(lambda x: 1 if keyword in x else 0)
                else:
                    v_articles['Keywords Similarity'] = 0
            ...
            v_articles = v_articles.sort_values( [ 'article_id',
                                                   'Keywords Similarity',
                                                   'User Articles Similarity', 
                                                   'User Articles Read',
                                                   'User Articles Interactions' ],
                                                 ascending = [True, False, False, False, False] )
            v_articles['Duplicate'] = v_articles.shift(1)['article_id'].fillna(-999)
            v_articles = v_articles[ v_articles['article_id'] != v_articles['Duplicate']]
            self.__recommendations__ = v_articles.drop('Duplicate', axis = 1).copy()
            ...
            if len(self.__keywords__) != 0:
                v_articles['Keywords Similarity'] = 0
                for keyword in self.__keywords__:
                    v_articles['topic_keywords_'] = v_articles['topic_keywords'].apply(lambda x: 1 if keyword in x else 0)
                    v_articles['Keywords Similarity'] = v_articles['Keywords Similarity'] + v_articles['topic_keywords_']
            ...
            self.__recommendations__ = v_articles.copy()
